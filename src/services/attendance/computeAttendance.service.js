...
async function computeMonthForImport(importId) {
  ...
  try {
    ...
    // ✅ Fetch Weekend Days setting (default to Friday = 5)
    const weekendSetting = await SystemSetting.findByPk("weekend_days", { transaction: t });
    let globalWeekendDaysArray = [5];
    let globalSaturdayOff = 'all';

    if (weekendSetting) {
      const v = weekendSetting.value;
      if (Array.isArray(v)) {
        globalWeekendDaysArray = v;
      } else if (v && typeof v === 'object') {
        globalWeekendDaysArray = v.days || [5];
        globalSaturdayOff = v.saturdayOff || 'all';
      }
    }
    const weekendDays = new Set(globalWeekendDaysArray);

    const excuseByEmpDate = new Map();
    const excuseStats = new Map();
    ...
    const approvedExcuseByEmpDate = new Map();
    const approvedLeaveByEmpDate = new Map();
    const approvedReqStats = new Map();
    ...
    for (const r of approvedRequests) {
      ...
      if (r.type === "leave_day") {
        ...
        approvedLeaveByEmpDate.set(key, lt);
      }
    }
    ...
    for (const [employeeId, empDays] of byEmp.entries()) {
      ...
      // ✅ Fetch Auth user for custom weekend policy
      const authUser = await Auth.findByPk(employeeId, {
      ...
      for (const d of empDays) {
        ...
        // ✅ Check if date is a weekend day based on individual or global policy
        const dayOfWeek = new Date(d.date).getDay(); // 0=Sun, ..., 5=Fri, 6=Sat

        let policyToUse = userWeekendPolicy;

        // Fallback to global if no user-specific policy exists
        if (!policyToUse) {
          policyToUse = {
            fridayOff: weekendDays.has(5),
            saturdayOff: globalSaturdayOff
          };
        }

        if (dayOfWeek === 5) {
          isWeekend = policyToUse.fridayOff;
        } else if (dayOfWeek === 6) {
          if (policyToUse.saturdayOff === 'all') isWeekend = true;
          else if (policyToUse.saturdayOff === 'none') isWeekend = false;
          else {
            const dateObj = new Date(d.date);
            const satNumber = Math.ceil(dateObj.getDate() / 7);
            const offCount = parseInt(policyToUse.saturdayOff);
            isWeekend = satNumber <= offCount;
          }
        } else {
        ...
        if (isPublicHoliday || isWeekend) {
          ...
          d.policyReason = isPublicHoliday
            ? `public_holiday: ${holidayMap.get(d.date) || ""}`
            : `weekend`;
        ...
        if (approvedLeaveType) {
          ...
          if (!isException && sickPenalty > 0) totalAbsentPenaltyDays += sickPenalty;
          await d.save({ transaction: t });
          continue;
        }
        ...
        if (approvedExcuseByEmpDate.has(dayKey)) excuseMin += Number(approvedExcuseByEmpDate.get(dayKey) || 0);
        ...
        const graceAvailable = !isException && effectiveLate > 0 && effectiveLate <= 15 && graceUsed < GRACE_MAX_COUNT;
        ...
        if (!isException) totalLatePenaltyDays += Number(r.penalty || 0);
        await d.save({ transaction: t });
      }
      ...
      const salaryBase = payroll?.netSalary ?? payroll?.net_salary ?? payroll?.grossSalary ?? payroll?.gross_salary ?? null;
      ...
      const loanInstallments = await LoanInstallment.findAll({
        where: { employeeId, month, status: { [Op.ne]: "skipped" } },
        transaction: t
      });
      const loanInstallmentAmount = loanInstallments.reduce((sum, inst) => sum + Number(inst.amount || 0), 0);

      // ✅ Fetch KPI Bonus (using already fetched authUser)
      let kpiBonus = 0;
      try {
        if (authUser) {
          const [year, m] = month.split('-').map(Number);
          const kpiRes = await KpiService.calculateMonthlyKpi(authUser.id, m, year);
          kpiBonus = kpiRes.totalEarned || 0;
        }
      } catch (err) {
        console.error(`KPI Calculation failed for emp ${employeeId}:`, err);
      }

      const finalNetSalary = Math.max(0, Number(salaryBase || 0) + Number(kpiBonus) - Number(deductionAmount || 0) - Number(loanInstallmentAmount));
      ...
      await AttendanceMonthlySummary.create(
        {
          ...
          manualAdjustmentAmount: manualDelta,
          finalNetSalary,
          status: 'draft',
          computedAt: new Date(),
        },
